...
% Binary filtering
se1 = strel('diamond', 3);       % 깎으면 붙어있는 쌀알이 없어지는 효과를 얻음.

% Apply erosion to the specified horizontal region
horizontalRegion = imgB(3:180, :);
horizontalRegion = imerode(horizontalRegion, se1);
imgB(3:180, :) = horizontalRegion;

se2 = strel('diamond', 2);       
% Apply erosion to the specified vertical region
vertitalRegion = imgB(:, 160:165);
vertitalRegion = imerode(vertitalRegion, se2);
imgB(:, 160:165) = vertitalRegion;
