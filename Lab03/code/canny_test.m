...
sigma = 1.5; % 표준 편차 값 조정
filtered_imgR = imgaussfilt(imgR, sigma);

% Display the filtered image
figure(6); imshow(filtered_imgR);
...
while (num < 1000)
    [maxval, r, c] = max2d(filtered_imgR);
    if maxval < threshold
        break;
    end
    num = num + 1;
    ...
    % Erase
    rs = max(1, r - radr);
    re = min(size(filtered_imgR, 1), r + radr);
    cs = max(1, c - radc);
    ce = min(size(filtered_imgR, 2), c + radc);
    filtered_imgR(rs:re, cs:ce) = 0;
end

rad = radr;
color = [255, 0, 0];    % Red
imgC = DrawCross(img, rcpnt, rad, color);
figure(7); imshow(imgC);
