...
% PerspectiveImage = histeq(PerspectiveImage);      % 히스토그램 균등화

% 변환된 이미지 출력
figure(2); imshow(PerspectiveImage);

PerspectiveImage = rgb2gray(PerspectiveImage);  % edge검출을 위해 gray이미지로 변환
figure(3); imhist(PerspectiveImage);

% Edge detection using Sobel & Canny filter
EdgeImage = edge(PerspectiveImage, 'sobel');
...
for i = 1:numBlocks(1)
    for j = 1:numBlocks(2)
        ...
        block = imgR(startIndexY:endIndexY, startIndexX:endIndexX);

        % 세로 엣지 검출
        verticalEdges = sum(block, 1);

        % 세로 엣지 수가 50개 이상인 경우
        if (sum(verticalEdges >= 1) >= 50)
            % 블록 중심 좌표 계산
